...
- Stream APIs: `Query#stream()` is adapted to `Connection::query_stream(...)` for pull-based typed `RowStream` rows and `Connection::query_stream_json(...)`, a lazy `polycpp::stream::Readable<Buffer>` of newline-delimited JSON chunks. `createBinlogStream` maps to `Connection::create_binlog_stream(...)`, a pull-based typed `BinlogStream` whose data chunks are consumed with `polycpp::stream::event::Data`. A connection with an active query stream rejects new commands until the stream reaches EOF or is destroyed/dropped, at which point remaining packets are drained to keep the protocol synchronized. A connection with an active binlog stream rejects new commands until EOF, `max_events`, or destruction; ending a live replication stream before EOF closes the transport because the server is still in replication packet mode.
