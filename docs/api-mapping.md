...
- Stream APIs: `Query#stream()` is adapted to `Connection::query_stream(...)` for pull-based typed `RowStream` rows and `Connection::query_stream_json(...)`, a lazy `polycpp::stream::Readable<Buffer>` of newline-delimited JSON chunks. A connection with an active query stream rejects new commands until the stream reaches EOF or is destroyed/dropped, at which point remaining packets are drained to keep the protocol synchronized.
...
- Unsupported or non-meaningful Node-specific APIs and audit reason: exact Node `createBinlogStream` object/EventEmitter shape remains adapted as documented in `docs/divergences.md`. Row object-mode query chunks now map to `RowStream`.
