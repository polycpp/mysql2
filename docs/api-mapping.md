...
| `Query#stream()` object-mode rows | `Connection::query_stream(...)`, `Connection::query_stream_json(...)` | adapted | `query_stream(...)` exposes `polycpp::stream::Readable<Row>` typed object chunks; `query_stream_json(...)` exposes an explicit newline-delimited JSON `polycpp::stream::Readable<Buffer>` serializer. |
...
- Stream APIs: `Query#stream()` is adapted to `Connection::query_stream(...)` for typed `polycpp::stream::Readable<Row>` rows and `Connection::query_stream_json(...)`, a `polycpp::stream::Readable<Buffer>` of newline-delimited JSON chunks.
...
- Unsupported or non-meaningful Node-specific APIs and audit reason: exact Node `createBinlogStream` object/EventEmitter shape remains adapted as documented in `docs/divergences.md`. Row object-mode query chunks now map to `polycpp::stream::Readable<Row>`.
