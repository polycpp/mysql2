...
- `Connection::query_each_raw(...)` is a C++-specific scan extension, not an upstream JavaScript object-row parity API. It exposes `RawRowView`/`RawValueView` packet byte views that are valid only during the callback and closes the transport if the callback throws before the result is drained.
