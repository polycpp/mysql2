...
- unsupported Node-specific APIs and audit reason: exact Node `Readable` object-mode row chunks, Unix socket server listen overloads, TLS server mode, GTID replication, continuous binlog object streams, and full row-event decoding are deferred because they require additional protocol families or polycpp stream/object primitives.
