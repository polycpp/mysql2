...
- stream APIs: upstream object-mode row streams map to `polycpp::stream::Readable<Row>` typed chunks; `query_stream_json()` exposes `polycpp::stream::Readable<Buffer>` newline-delimited JSON chunks for byte-stream consumers.
...
- unsupported Node-specific APIs and audit reason: exact Node `createBinlogStream` object/EventEmitter shape is adapted to bounded reads, callback-controlled reads, and explicit parser state. Query row object-mode chunks now map to `polycpp::stream::Readable<Row>`.
